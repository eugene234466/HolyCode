...
// ─── LEARN CONCEPT — from data attribute ───
...
const SUBSCRIPTION_KEY = 'holycode_sub_id';

if (notifyBtn) {
    // Check if already subscribed
    const existingSubId = localStorage.getItem(SUBSCRIPTION_KEY);
    if (existingSubId) {
    ...
    notifyBtn.addEventListener('click', async () => {
        ...
        if (existingSubId) {
            try {
                await fetch('/unsubscribe', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id: existingSubId })
                });
                ...
                showToast('Could not unsubscribe. Try again.');
            }
            return;
        }
        ...
        try {
            ...
            if (!vapidPublicKey) {
                showToast('Notifications set up! 🔔');
                return;
            }
            ...
            if (data.success) {
                // Store subscription ID for unsubscribe
                localStorage.setItem(SUBSCRIPTION_KEY, data.id);
                notifyBtn.textContent = '🔕 Unsubscribe';
                notifyBtn.classList.add('subscribed');
